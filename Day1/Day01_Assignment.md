...
Public Cloud : 인터넷을 통해 누구나 접근 가능한 형태이다. (AWS 등)

장점 : 초기 비용이 저렴하고, 확장성이 뛰어나다.

단점 : 보안적인 측면에서 민감한 데이터는 적합하지 않다.

제공 방식 : 인터넷을 통해 접근 가능하도록 서비스 제공자가 인프라를 구성한다.

Private Cloud : 특정 기업, 조직에서만 사용하는 클라우드 환경이다.

장점 : 보안과 커스터마이징이 뛰어나다.

단점 : 구축과 운영 비용이 높다.

제공 방식 : 물리적인 데이터 센터를 자체적으로 운영하는 것이 대표적이다.

Hybrid Cloud : Public과 Private을 합친 상태이다.

장점 : 민감한 데이터는 내부에서 관리하고, 나머지는 퍼블릭에서 활용하여 유연하게 사용할 수 있다.

단점 : 관리와 운영이 어렵다.

제공 방식 : Public Cloud와 Private Cloud 을 혼합하여 필요한 자원을 적절히 배분해 활용하는 방식으로 제공한다.
...
<img width="763" height="559" alt="image" src="https://github.com/user-attachments/assets/3f8df87a-5a8b-48e6-abe3-954795d0b451" />
...
## Well-Known Ports와 프로토콜

### Port

1. 80 (HTTP)
    - 웹사이트 접속에 사용되는 기본적인 포트이다.
    - 암호화 없이 데이터를 주고 받는다.
        
        HTTP : HyperText Transfer Protocol으로 클라이언트와 서버 간 데이터를 주고받는 프로토콜이다.
        
        - 암호화가 없어 보안에 취약하다.
        
2. 443 (HTTPS)
    - HTTP에 SSL/TLS 암호화를 적용한 보안 웹 통신 포트이다.
    - 은행 등 보안이 필요한 사이트에서 사용한다.
        
        SSL/TLS : 통신 데이터를 암호화 하고, 웹사이트의 신원을 확인하여 안전한 인터넷 연결을 제공하는 보안 프로토콜이다.
        
    
3. 22 (SSH)
    - 원격 접속을 위한 보안 프로토콜 포트이다.
    - 서버 관리, 터미널 접속시 사용한다.
        
        SSH : 두 컴퓨터간 네트워크 통신을 암호화 하여 안전하게 연결하는 프로토콜이다.
...
4. 25 (SMTP)
    ...
    - 메일 서버 간 메일 전송시 사용한다.
        ...
        SMTP : Simple Mail Transfer Protocol의 약자로 단순한 메일 전송 프로토콜 이다.
        
    
5. 53 (DNS)
    - 도메인 이름을 IP주소로 변환하는 포트이다.
    - 인터넷에서 주소를 찾아 접속할 때 필수이다.
        
        DNS : Domain Name System의 약자로 사용자가 기억하기 쉬운 도메인 이름을 컴퓨터가 이해할 수 있는 IP로 변환하는 인터넷 시스템이다.
